...
public class WindowChangeDetectingService extends AccessibilityService {
    private static final String TAG = WindowChangeDetectingService.class.getName();
            ...
            if (isActivity) {
                for (int i = 0; i < MainActivity.mApps.size(); i++) {
                    if (componentName.getPackageName().contains("monitor")) {
                        ...
                        if (MainActivity.mApps.get(i).getIntent().getPackageName().equals(componentName.getPackageName()))//获得栈顶进程
                        {
